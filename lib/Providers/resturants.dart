  ...
  Future<String> deletePhoto(
    ...
    try {
      ...
      QuerySnapshot allDocuments = await databaseReference
          ...
          .document(productId)
          .collection(photosForSpecificCategory.type)
          .where('date', isEqualTo: photosForSpecificCategory.date)
          .where('userId', isEqualTo: userId)
          .getDocuments();
...
//        });
  ...
  Future<String> startAReview(
    ...
    if (comment != null || ratingCount.trim() != '0.0' || imageFile != null) {
      ...
      CollectionReference userActivity = databaseReference
          ...
          .document(userId)
          .collection('activities');
      CollectionReference userComments = databaseReference
          .collection('users')
          .document(userId)
          .collection('comments');
      // var subCollection = mainCollection.document(productId).collection('photos');
      ...
      try {
        ...
        String date = DateTime.now().toIso8601String();
        await categoryCollection.document().setData({
          'userId': userId,
          'imgUrl': imgUrl.toString(),
  ...
  Future<String> deleteAReview(
    ...
    CollectionReference categoryCollection = databaseReference
        ...
        .document(categoryId)
        .collection('comments');
    var allUsersWhichMakeAction =
        await databaseReference.collection('users').getDocuments();
    DocumentSnapshot specificUserWhichMakeAction;
    try {
      await categoryCollection.document(review.id).delete();
      for (int i = 0; i < allUsersWhichMakeAction.documents.length; i++) {
        specificUserWhichMakeAction = await databaseReference
            .collection('users')
            .document(allUsersWhichMakeAction.documents[i].documentID)
            .collection('comments')
            .document('$categoryId${review.id}')
            .get();
        if (specificUserWhichMakeAction.exists) {
          await databaseReference
              .collection('users')
              .document(allUsersWhichMakeAction.documents[i].documentID)
              .collection('comments')
        ...
        if (doc.documents.length >= 1) {
          ...
          for (int i = 0; i < doc.documents.length; i++) {
            document = await collection
                ...
                .get();
            var isUserMakeAction = await collection
                .document(userId)
                .collection('comments')
                .document('${category.id}${doc.documents[i].documentID}')
                .get();
            bool isFunny = false;
            bool isCool = false;
            bool isUseful = false;
            if (isUserMakeAction.exists && isUserMakeAction != null) {
              isFunny = isUserMakeAction.data['isFunny'] ?? false;
              isCool = isUserMakeAction.data['isCool'] ?? false;
              isUseful = isUserMakeAction.data['isUseful'] ?? false;
            }
...
//    bool isUseful=false;
    ...
    CollectionReference categoryCollection = databaseReference
        ...
        .document(category.id)
        .collection('comments');
    var isUserMakeAction = databaseReference
        .collection('users')
        .document(userId)
        .collection(
            'comments'); //.document('${category.id}${review.id}').get();
//    if(isUserMakeAction.exists || isUserMakeAction.data.isNotEmpty){
//      isFunny =isUserMakeAction.data['isFunny']??false;
//      isCool =isUserMakeAction.data['isCool']??false;
//      isUseful=isUserMakeAction.data['isUseful']??false;
//    }
    ...
    int funny = 0;
    var getAllAction =
        await isUserMakeAction.document('${category.id}${review.id}').get();
    if (getAllAction.exists && getAllAction != null) {
      useful = getAllAction.data['useful']??0;
      cool = getAllAction.data['cool']??0;
      funny = getAllAction.data['funny']??0;
    }
//      isFunny =isUserMakeAction.data['isFunny']??false;
//      isCool =isUserMakeAction.data['isCool']??false;
//      isUseful=isUserMakeAction.data['isUseful']??false;
//    }
    if (type == 'useful') {
      ...
      if (review.isUseful ==true && useful == 0) {
        usefulNum = usefulNum + 1;
        useful = 1;
      }
      if(review.isUseful ==false && useful == 1){
        if (usefulNum > 0) {
          usefulNum = usefulNum - 1;
          useful =0;
        }
      ...
      if (review.isFunny==true && funny == 1) {
        review.isFunny = false;
        funnyNum = funnyNum - 1;
        funny =0;
      }
      if (review.isColl ==true && cool ==1) {
        review.isColl = false;
        coolNum = coolNum - 1;
        cool =0;
      }
      ...
      if (review.isColl ==true && cool ==0) {
        coolNum = coolNum + 1;
        cool =1;
      }
      if(review.isColl ==false && cool ==1){
        if (coolNum > 0) {
          cool =0;
          coolNum = coolNum - 1;
        }
      ...
      if (review.isUseful == true && useful == 1) {
        useful = 0;
      ...
      if (review.isFunny ==true && funny == 0) {
        funnyNum = funnyNum + 1;
        funny = 1;
      }
      if (review.isFunny ==false && funny == 1){
        if (funnyNum > 0) {
          funnyNum = funnyNum - 1;
          funny=0;
        }
    ...
    await isUserMakeAction.document('${category.id}${review.id}').setData({
...
//    }
  ...
  Future<List<Category>> getNearbyPlaces(
      ...
      double radius = 2000,
      bool isCommingFromNearby = false}) async {
    try {
      List<Category> _rest = [];
          ...
          if (distance <= radius && place.data['bussinessCategory'] == type) {
            ...
            _rest.add(Category(
              ...
              priceLevel: place.data['priceLevel'] != null
                  ...
                  : '',
              imgUrl:
                  place.data['imgUrl'] != null ? [place.data['imgUrl']] : [],
